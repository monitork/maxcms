...
class Banner_position extends MX_Controller {
		...
		if($_POST){
			...
			if($_FILES){
				foreach($fileName as $k=>$v){
					if(isset($_FILES['fileAdmincp']['error'][$k])){
						...
						if(strstr($_FILES['fileAdmincp']['type'][$k],strtolower($type))){
							$tmp_name[$k] = $_FILES['fileAdmincp']["tmp_name"][$k];
							$file_name[$k] = $_FILES['fileAdmincp']['name'][$k];
							$ext = strtolower(substr($file_name[$k], -4, 4));
							if($ext=='jpeg'){
								$fileName[$k] = time().'_'.create_slug(substr($file_name[$k],0,-5)).'.jpg';
							}else{
								$fileName[$k] = time().'_'.create_slug(substr($file_name[$k],0,-4)).$ext;
							}
						}else{
							print 'Tập tin tải lên sai định dạng .';
							exit;
						}
					}
				}
			...
			if($this->model->saveManagement($fileName)){
				if($_FILES){
					foreach($fileName as $k=>$v){
						if(isset($_FILES['fileAdmincp']['error'][$k])){
							$upload_path = BASEFOLDER.'static/uploads/banner/'.$dir;
